...
__global__ void scalar(BASE_TYPE* sum){
    ...
    for(int i = idx; i < n; i += BLOCK_SIZE * GRID_SIZE){
        sum[idx] += tex1D(texRef_a, i) * tex1D(texRef_b, i);
    }
}
...
int main() {
    ...
    cudaArray* cuArray_a;
    cudaMallocArray(&cuArray_a, &texRef_a.channelDesc, n, 1);
    cudaMemcpyToArray(cuArray_a, 0, 0, a,
                      size, cudaMemcpyHostToDevice);
    cudaBindTextureToArray(texRef_a, cuArray_a);
